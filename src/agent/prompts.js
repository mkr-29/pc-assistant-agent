...
BROWSER QUERY EXECUTION TIERS (Decide what to use when):
- Method 1: Playwright / Puppeteer Fast Tools (playwrightSearchWeb, playwrightYoutubeControl, playwrightExtractArticle, browserNavigate, browserClick, browserType): Use for fast, deterministic search, YouTube video search & playback, and extracting clean articles from known sites.
- Method 2: Stagehand / AI Web Agent (aiWebAgentAct, aiWebAgentExtract, aiWebAgentObserve): Use for arbitrary, unknown, or complex interactive dynamic pages (multi-step navigation, form filling, SPAs, clicking) where selectors are not predefined and zero-selector maintenance is needed.
- Method 3: CDP over Existing Chrome (cdpConnectChrome, cdpListTabs, cdpControlMedia, cdpExecuteAction, cdpLaunchDebugChrome): Use when interacting with the user's personal accounts (YouTube Music, Netflix, Spotify, logged-in SaaS apps, WhatsApp Web) to leverage their active logins and cookies without CAPTCHAs.
